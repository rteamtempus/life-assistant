...
//
// Emits two migrations:
//   * 0013_tarot_seed_reference.sql — all 78 RWS-78 cards; the built-in
//     "Original (RWS-based)" set row; the Rider-Waite-Smith deck + card_image
//     mappings; the built-in spreads (Three-Card, Celtic Cross, Cross of Kells);
//     the tarot_principles AI scaffolding.
//   * 0014_tarot_seed_interpretations.sql — upright/reversed text + keywords for
//     all 78 cards (from interpretations.json).
//
// Both are idempotent (ON CONFLICT upserts). Applied via `supabase db push`
// like any other life-assistant migration.
// ============================================================
import { writeFileSync, readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';

// Outputs are life-assistant migrations (applied via `supabase db push` now
// that tarot is part of this app/repo). Edit interpretations.json / the
// canonical lists below, then re-run `npm run seed:build`.
const HERE = dirname(fileURLToPath(import.meta.url));
const MIGR = join(HERE, '..', 'migrations');
const OUT = join(MIGR, '0013_tarot_seed_reference.sql');
const OUT_INTERP = join(MIGR, '0014_tarot_seed_interpretations.sql');
const INTERP_JSON = join(HERE, 'interpretations.json');
...
// Postgres text[] literal, safe for both " (escaped) and ' (doubled) inside elements.
const arr = (a) => {
  const body = (a ?? []).map((k) => `"${String(k).replace(/"/g, '\\"')}"`).join(',');
...
// ---------- emit SQL ----------
let sql = `-- 0013_tarot_seed_reference.sql — GENERATED by supabase/seed/build-seed.mjs. Do not edit by hand.
...
// ============================================================
// Interpretations migration — built from interpretations.json into the
// built-in "Original (RWS-based)" set. The JSON's `set_id`/`name` fields are
// ignored (set_id is forced to SET_ORIGINAL; name lives on tarot_cards).
// ============================================================
const cardIds = new Set(cards.map((c) => c.id));
const interp = JSON.parse(readFileSync(INTERP_JSON, 'utf8'));

const unknown = interp.filter((r) => !cardIds.has(r.card_id));
if (unknown.length) throw new Error(`Unknown card_id(s) in interpretations.json: ${unknown.map((r) => r.card_id).join(', ')}`);
const seenIds = new Set(interp.map((r) => r.card_id));
if (seenIds.size !== interp.length) throw new Error('Duplicate card_id in interpretations.json');

let isql = `-- 0014_tarot_seed_interpretations.sql — GENERATED by supabase/seed/build-seed.mjs.
-- The "Original (RWS-based)" interpretation set: upright/reversed text + keywords
-- for all ${interp.length} cards. Idempotent (ON CONFLICT). Do not edit by hand;
-- edit interpretations.json and re-run \`npm run seed:build\`.

begin;

insert into tarot_interpretations
  (set_id, card_id, upright_text, upright_keywords, reversed_text, reversed_keywords) values
${interp
  .map(
    (r) =>
      `  (${q(SET_ORIGINAL)}, ${q(r.card_id)}, ${q(r.upright_text)}, ${arr(r.upright_keywords)}, ${q(r.reversed_text)}, ${arr(r.reversed_keywords)})`,
  )
  .join(',\n')}
on conflict (set_id, card_id) do update set
  upright_text = excluded.upright_text, upright_keywords = excluded.upright_keywords,
  reversed_text = excluded.reversed_text, reversed_keywords = excluded.reversed_keywords,
  updated_at = now();

commit;
`;

writeFileSync(OUT_INTERP, isql);
console.log(`Wrote ${OUT_INTERP}`);
console.log(`  ${interp.length} interpretations.`);
if (interp.length !== 78) console.warn(`WARNING: expected 78 interpretations, got ${interp.length}.`);
